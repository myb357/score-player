...
- 新建谱子：多图上传并拖拽排序、逐页设置翻页时间点、两种翻页模式、伴奏上传（音频或视频，视频自动用 ffmpeg 提取音轨）、音频在线剪辑（ffmpeg 裁剪片段）、节拍器 BPM / 0.01 秒时间轴偏移 / 音量配置，支持高级节拍分析基于整首伴奏推荐 BPM 和偏移；已填写 BPM 时只推荐偏移，试听选段时同步播放节拍器用于对齐伴奏。
