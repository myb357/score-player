...
`static/new.html` 是新建和编辑谱子的页面。它支持谱子命名、选择翻页模式、多图片上传、拖拽排序、图片自动裁边后手动裁剪、上传音频或视频、截取伴奏片段、AI 分析伴奏鼓点推荐节拍器偏移、试听选段时同步播放节拍器，以及编辑已有谱子时保留、替换或移除伴奏。节拍器配置包含 BPM、0.1 秒粒度的时间轴偏移和音量，保存到谱子设置内。
