...
镜像主来源为阿里云 ACR：`crpi-rd0vl6t3c1p11agm.cn-beijing.personal.cr.aliyuncs.com/myb357/score-player:latest`。GitHub Actions 同时推送 GHCR：`ghcr.io/myb357/score-player:latest`，作为备用镜像源。软路由上的 Watchtower 每 5 分钟检查并自动拉取新镜像更新 `sp-app`。此外 CI 采用「Cloudflare Webhook 主动部署」：镜像推送完成后，GitHub Actions 经 Cloudflare Tunnel 暴露的 `https://webhook.scoreplayer-myb.top/deploy?token=<WEBHOOK_TOKEN>` 向软路由 `sp-webhook` 容器发起 POST 请求，`sp-webhook`（`deploy/softrouter/webhook/server.py`，Python 标准库实现，监听 `9003`）校验 token 后在本地执行 `docker pull` 与 `docker-compose up -d --no-deps score-player`。`sp-webhook` 直接挂载宿主机 `/usr/bin/docker` 与 `/usr/bin/docker-compose`，启动时打印探测到的路径，不再通过 `apk` 动态安装工具链。该链路依赖 GitHub 仓库 Secret `WEBHOOK_TOKEN` 与软路由 `.env` 的 `WEBHOOK_TOKEN` 保持一致。原基于 Tailscale + SSH 的自动部署方案（含 `SOFTROUTER_SSH_KEY`、`TAILSCALE_AUTH_KEY`）已移除。
