...
播放器倍速功能已实现，支持 0.1x 到 2.0x 范围内调整播放速度；为降低 Android WebView / Chromium 变速保调造成的金属感和糊音，播放页会关闭 `preservesPitch` 并使用原生重采样优先保证伴奏清晰度。播放器伴奏音量增益已实现，基于 Web Audio GainNode 在网页内将伴奏输出提升到 1.0x 到 3.0x，不修改系统音量。
